...
// Temporary test endpoint to generate missing months (remove in production)
router.post("/generate-missing-months", async (req, res) => {
  try {
    const { generatePayslipsForMonth } = await import("../services/payslip-bulk.service");
    const results = [];
    
    // Generate payslips for May through August 2026
    const months = [
      { month: 5, year: 2026, name: "May" },
      { month: 6, year: 2026, name: "June" }, 
      { month: 7, year: 2026, name: "July" },
      { month: 8, year: 2026, name: "August" }
    ];
    
    for (const { month, year, name } of months) {
      console.log(`Generating payslips for ${name} ${year}...`);
      try {
        const result = await generatePayslipsForMonth(month, year);
        results.push({ month: name, year, success: true, result });
        console.log(`✅ ${name} ${year} completed: ${result.successfulPayslips} payslips generated`);
      } catch (error: any) {
        results.push({ month: name, year, success: false, error: error.message });
        console.error(`❌ ${name} ${year} failed:`, error.message);
      }
    }
    
    res.json({
      message: "Missing months generation completed",
      results
    });
  } catch (error: any) {
    console.error("Generate missing months failed:", error);
    res.status(500).json({
...
// Temporary test endpoint to check existing payslips (remove in production)
router.get("/check-payslips/:employeeId", async (req, res) => {
  try {
    ...
    res.json({
      ...
      payslips: payslips.map(p => ({
        ...
        month: p.payrollRun?.payrollMonth || 'unknown',
        year: p.payrollRun?.payrollYear || 'unknown',
        netSalary: Number(p.netSalary || 0),
        totalEarnings: Number(p.totalEarnings || 0),
        status: p.status,
        generatedAt: Number(p.generatedAt)
      }))
    });
  } catch (error: any) {
